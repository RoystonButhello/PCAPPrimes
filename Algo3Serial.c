...
int main()
{
	...
	_Bool *C = (_Bool *)calloc(len, sizeof(_Bool));
	...
	START_TIME = clock();
	for (i = 3; i <= (integer)pow(n, 0.5); i += 2)					//i->[3,5,7,9...,sqrt(n)] | i corresponds to C[(i-3)/2]
	{
		if (C[(i - 3) / 2] == 0)		//false implies <i> is prime
		{
			for (j = (integer)pow(i, 2); j <= n; j += (2 * i))	//j->[i^2,n] | increments by 2*i
				C[(j - 3) / 2] = 1;
			/*LOGIC
