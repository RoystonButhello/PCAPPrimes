...
__global__ void SieveBlock(uint32 *P, uint64 *mark, uint64 m, uint32 completed, uint32 plen, uint32 segsize, bool print)
{
	...
	//Print last found prime for last set of segments
	if (print)
	{
		for (j = max; j >= min; j -= 2)
		{
...
int main(uint32 argc, char* argv[])
{
	...
	{
		if (completed + 1 == segments)	print = true;
		cudaMemcpy(dmark, mark, (m / 128) * size, cudaMemcpyHostToDevice);
		cudaEventRecord(start);
		SieveBlock << <1, TPB >> > (dP, dmark, m, completed, plen, segsize, print);
