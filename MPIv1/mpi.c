...
void sieveBlock(uint32 *, uint64, uint32, uint32, uint32, int);
...
int main(int argc, char *argv[])
{
	...
	{
		START_TIME = clock();
		sieveBlock(P, m, plen, seglen, completed, rank);
		END_TIME = clock();
...
void sieveBlock(uint32 *P, uint64 m, uint32 plen, uint32 seglen, uint32 completed, int rank)
{
	int pid;
	uint32 prime;
	uint64 i, j, minb;
	uint64 min = (uint64)completed * m + ((uint64)rank * (uint64)seglen) + 1;
	uint64 max = min + seglen - 2;
	///printf("Segment %3u.%d sieving %llu->%llu | min = %u*%llu + %d*%u + 1\n", completed, rank, min, max, completed, m, rank, seglen);

	prime = P[0];
	uint64 *mark = (uint64 *)calloc(seglen / 128, size64);
